...
def test_replay_overlay_injected_when_counts_present(tmp_path: Path):
    ...
    _inject_replay_feature_overlay(svg, stair_count=2, room_count=3)
    text = svg.read_text(encoding="utf-8")
    assert "Replay feature overlay | Stairs: 2 | Rooms: 3" in text
...
def test_replay_overlay_not_injected_when_counts_zero(tmp_path: Path):
    ...
    _inject_replay_feature_overlay(svg, stair_count=0, room_count=0)
...
def test_replay_overlay_respects_profile_feature_toggles(tmp_path: Path):
    ...
    overlay = replace(profile.floor_plan.feature_overlay, rooms_enabled=False)
    _inject_replay_feature_overlay(svg, stair_count=2, room_count=3, overlay_style=overlay)
    text = svg.read_text(encoding="utf-8")
    assert "Replay feature overlay | Stairs: 2 | Rooms: off" in text
    assert ">D</text>" not in text
    assert ">UP</text>" in text
...
def test_replay_overlay_renders_symbols_from_serialized_view_anchors(tmp_path: Path):
    ...
    text = svg.read_text(encoding="utf-8")
    assert "Replay feature overlay | Stairs: 1 | Rooms: 1" in text
    assert ">D</text>" not in text
    assert ">UP</text>" in text
...
def test_replay_overlay_skips_duplicate_symbols_when_sheet_already_has_feature_overlay(tmp_path: Path):
    svg = tmp_path / "sheet.svg"
    svg.write_text(
        '<svg width="100" height="100"><text>Feature overlay | Stairs: 1 | Rooms: 0</text></svg>\n',
        encoding="utf-8",
    )
    _inject_replay_feature_overlay(
        svg,
        ...
        view_overlay={
            "bounds": {"min_x": 0.0, "min_y": 0.0, "max_x": 10.0, "max_y": 10.0},
            "feature_anchors": [
                {"ifc_class": "IfcStair", "anchor": {"x": 1.0, "y": 1.0}, "dir_x": 1.0, "dir_y": 0.0, "source_element": "s-1"},
            ],
        },
    )
    text = svg.read_text(encoding="utf-8")
    assert "Replay feature overlay | Stairs: 1 | Rooms: 0" in text
